...
import { 
  ...
  updateGembaData, 
  deleteGembaData 
} from '../services/firebaseGembaService';

const DataFormGemba = ({ data, onDataChange }) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      const savedData = await saveGembaData(newData);
  ...
  const handleDelete = async (index) => {
    ...
    setSyncStatus('saving');
    try {
      await deleteGembaData(itemToDelete.id);
      const newData = data.filter((_, i) => i !== index);
      onDataChange(newData);
      setSyncStatus('saved');
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      if (editingId) {
        await updateGembaData(editingId, updatedData);
        const newData = [...data];
        newData[editingIndex] = { ...updatedData, id: editingId };
        onDataChange(newData);
      } else {
        const savedData = await saveGembaData(updatedData);
        const newData = [...data];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">
          {editingIndex !== null ? '✏️ Modifier la semaine' : '➕ Ajouter une semaine Gemba'}
        </h3>
      ...
      {data.length === 0 && (
        <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
          <p className="text-gray-500">Aucune donnée Gemba OJT</p>
