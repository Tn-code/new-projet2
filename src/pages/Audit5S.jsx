...
const Audit5S = ({ currentWeek, onWeekChange }) => {
  ...
  const validData = auditData.filter(row => row.planifie > 0 || row.realise > 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">🏭 Audit 5S</h1>
          <p className="text-sm text-gray-500">Suivi des audits 5S - Données sauvegardées dans Firebase</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
          <Database className="w-4 h-4" />
          Saisie des données Audit 5S
        </h3>
        <DataForm data={auditData} onDataChange={handleDataChange} />
      </div>
